...
const ModelComparison = ({ predictions, data, models, backtestResultsAll }) => {
  ...
  lineTraces.push({
    x: histData.map(d => d.datetime), y: histData.map(d => d.close),
  ...
  validPreds.forEach(([key, pred], idx) => {
    ...
    const cfg = pred.config || models[key] || {};
    lineTraces.push({
      x: df.map(d => d.datetime), y: df.map(d => d.close),
  ...
  if (backtestResultsAll && Object.keys(backtestResultsAll).length > 0) {
    Object.entries(backtestResultsAll).filter(([, r]) => !r.error).forEach(([key, res]) => {
      ...
      lineTraces.push({
        x: (res.pred_df || []).map(d => d.datetime), y: (res.pred_df || []).map(d => d.close),
    ...
    if (firstRes && firstRes.actual_df) {
      lineTraces.push({
        x: firstRes.actual_df.map(d => d.datetime), y: firstRes.actual_df.map(d => d.close),
  ...
  validPreds.forEach(([key, pred], idx) => {
    ...
    candleTraces.push({
      x: df.map(d => d.datetime), open: df.map(d => d.open || d.close), high: df.map(d => d.high || d.close),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl p-6 border border-white/5">
        ...
        <Plot data={candleTraces} layout={{...darkLayout, title: 'Price Predictions Comparison (Candlestick)', height: 700, xaxis: { rangeslider: { visible: true } }}} config={plotConfig} className="w-full" useResizeHandler />
      </div>
    </div>
  );
};
